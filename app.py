    ...
    if not openai_api_key or not finnhub_api_key:
        ...
        if len(selected_tickers) == 1:
            ...
            for i, ticker in enumerate(selected_tickers):
                asset_type = detect_asset_type(ticker)
                status_text.text(T["status_analyzing"].format(ticker=ticker, asset_type=asset_type, i=i+1, total=len(selected_tickers)))
            ...
            status_text.text(T["multiple_analysis_completed"])
